...
readers-own [ ; we are serving a complete truck, at the moment
  ...
  token-positive ; true or false
  token-negative ; true or false
]
  ...
  if tpe = "read" [
    ...
    foreach use-coords [
      ...
      create-readers 1 [
        ...
        set token-negative false
        set token-positive false
...
;  let f-inv-max 0.053 ;cycles/s
  ...
  ask readers[
    ...
    if reader-utl < 0.5 [
      ; set delta (max (list 1 (delta - 5)) )
      if (random-float 1) <= (1 - reader-utl) [ set token-negative true ]
    ]
    if reader-utl > 0.9 [
      ;set delta (max (list 1 (delta + 5)) )
      if (random-float 1) <= (reader-utl) [ set token-positive true ]
    ]
  ]

  ; Now Exchange tokens
  let positive-readers (readers with [token-positive])
  let negative-readers (readers with [token-negative])
  if any? positive-readers [
    ask positive-readers [
      if any? negative-readers [
        ask one-of negative-readers [
          set delta (max (list 1 (delta - 5)) )
          set token-negative false
          set negative-readers (readers with [token-negative])
        ]
        set delta (max (list 1 (delta + 5)) )
      ]
    ]
  ];if
  ; token not used are lost
  ask readers [(set token-positive false) (set token-negative false) ]
...
<experiments>
  ...
  <experiment name="test3_token" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>avg-cycles-of-sns</metric>
    <metric>avg-cycles-of-io</metric>
    ...
    <enumeratedValueSet variable="readers-control-period">
      <value value="90"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="min-power-to-feed-tag-IC">
      <value value="-13"/>

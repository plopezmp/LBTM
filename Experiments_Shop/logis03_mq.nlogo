...
tags-own [
  ...
  ;tag-gain ;<----OJO QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR QUITAR
  tag-gain-fw
  tag-gain-bw
  tag-sensitivity
  q_e_fw q_e_bw
...
globals [
  ...
  cond_1 cond_2 ; for if of go proc.
  total-tag-queue-time
  avg-queue-length-tags
]
...
to setup-globals
  ...
  ifelse multi-size-batches? [
    set cond_1 10000 set cond_2 1000
  ][
    set cond_1 10000 set cond_2 60000
    set mean-arrival-rate (mean-arrival-rate * 1525.5)
  ]
  ;;
...
;  ]
  ;set distance-var-x 4 set distance-var-y 50 ; [12 22]
  ;set readers-per-row 10 set readers-per-column 5
  ;;
...
to check-queue-length-and-write
  ...
  if ticks >= (10 * num-of-queue-samples) and (num-of-queue-samples <= 1000) [
    ; if each 500 then <= 50; it depends of the max-num-of-iterations this example are 25000
    ;;--
;    let len length queue
;    let t ticks
;    let avg1 ifelse-value (t > 0.1)  [( total-tag-queue-time / (t ) )][0]
;    let avg2 ifelse-value (t > 0.1)  [( total-truck-queue-time / (t ) )][0]
;    set queue-length lput (list ticks len pallets-in-queue tags-in-queue avg1 avg2 ) queue-length
;    set num-of-queue-samples (num-of-queue-samples + 1)
    ...
    file-write ticks
    file-write len
    file-write pallets-in-queue
    file-write tags-in-queue
    file-write (ifelse-value (t > 0.1)  [( total-truck-queue-time / (t ) )][0]) ;avg-queue-length
    file-write (ifelse-value (t > 0.1)  [( total-tag-queue-time / (t ) )][0]) ;avg-queue-length-tags
    file-write (ifelse-value (total-queue-throughput > 0) [total-time-in-queue / total-queue-throughput] [0]) ; avg-time-queue-of-tags
    file-print ""
    file-close
    set num-of-queue-samples (num-of-queue-samples + 1)
    ;;--
...
to write-queue-file [ f-queue ]
  ...
  foreach range ( length queue-length ) [ ? ->
    ...
    file-write ( item 3 item ?  queue-length ) ; total tags in queue
    file-write ( item 4 item ?  queue-length ) ; avg of trucks in queue
    file-write ( last item ? queue-length )    ; avg of tags in queue
    file-print ""
  ]
  file-close
end
...
to go
  ...
  ;ifelse ticks < max-run-time and ( length queue <= 1500) [
  ifelse ticks <= cond_1 and ( length queue <= cond_2) [
    ...
    ;show next-event
    ;show (word "1= " (first next-event) " , 2= " (last next-event))
...
to-report var-frame-size
  ...
  ifelse tgs < 2 [ set k 1 ][
        ifelse  tgs < 4 [ set k 2 ][ ;Q1
          ifelse tgs < 7 [ set k 4 ][;Q2
            ifelse tgs < 12 [ set k 8 ][;Q3
              ifelse tgs < 23 [ set k 16 ][;Q4
                ifelse tgs < 45 [ set k 32 ][;Q5
                  ifelse tgs < 90 [ set k 64 ][;Q6
                    ifelse tgs < 178 [ set k 128 ][ ;Q7
                      ifelse tgs < 356 [ set k 256 ][ ;Q8
                        ifelse tgs < 711 [ set k 512 ][;Q9
...
to-report my-FSA
  ...
  set efficiency ifelse-value ((cs-t + cc-t + ce-t) > 0) [ cs-t / (cs-t + cc-t + ce-t)] [ efficiency ]
...
to-report one-frame-inventory [ reading-time ]
  ...
  ifelse any? tagson [   ;show but-first range  (frame-size + 1)
    ...
    foreach but-first range ( frame-size + 1 ) [ [ ?slot ] ->
      ...
      ask tagson [ compute-power-rec-in-tag reading-time frame-time ];<---------note
      ...
      ifelse any? contending-tags [
        ...
        ;ask contending-tags [ compute-power-rec-in-tag reading-time frame-time ];<---------note
      ] [set s 0 ]
      ;show (list ?slot s)
...
to-report gen-num-tags-and-height
  ; tags for  756, 176, 48, 70, 63, 54, 30, 36, 20, 12, 10 BOXES in pallet
  ;let p [ 0.06 0.15 0.2 0.1 0.05 0.08 0.05 0.15 0.1 0.03 0.03 ]
  ;let p [ 0 0.15 0.2 0.1 0.05 0.08 0.05 0.15 0.1 0.03 0.09 ]
  ;let p [ 0.01 0.08 0.2 0.1 0.1 0.1 0.05 0.15 0.1 0.08 0.03 ]
  ;let p [ 0.005 0.01 0.08 0.02 0.15 0.1 0.1 0.1 0.135 0.15 0.15 ]
  ;let p [ 0.01 0.04 0.1 0.13 0.15 0.17 0.1 0.1 0.1 0.05 0.05 ]
  ;let p [ 0.01 0.04 0.1 0.05 0.1 0.1 0.08 0.135 0.1 0.135 0.15 ] ; debe llegar a 0.55
  ;let p [ 0.02 0.04 0.1 0.06 0.1 0.1 0.08 0.2 0.1 0.1 0.1 ] ; debe llegar a 0.46
  ;let p n-values 11 [(1 / 11)] ; 0.35 satura
  ; let p [0.01 0.05 0.08 0.1 0.1 0.1 0.14 0.12 0.1 0.1 0.1] ; hasta 0.
  ;;
  ;let p [0.02 0.05 0.08 0.12 0.15 0.15 0.15 0.12 0.06 0.05 0.05] ;hasta 0.44 de lambda
  ;let probs [ [[756 1.946] ] [[176 1.892] ] [[70 1.995] ] [[63 1.862] ] [[54 1.980] ] [[48 1.986] ]
  ;   [[36 1.761] ] [[30 1.960 ] ] [[20 2] ] [[12 1.755] ] [[10 1.374] ] ]
  ;;
  let p [0.01 0.04 0.08 0.12 0.16 0.15 0.15 0.13 0.06 0.05 0.05] ;hasta 0.44 de lambda
  let probs [ [ [756 1.946] ] [[176 1.892] ] [[48 1.986] ] [[70 1.995] ]
    [[63 1.862] ] [[54 1.980] ] [[30 1.960 ] ] [[36 1.761] ] [[20 2] ] [[12 1.755] ] [[10 1.374] ] ]
  let probs-height (map [ [?1 ?2] -> lput ?1 ?2 ] p probs) ; assigns a probability to each [ntags p-height]
  report first rnd:weighted-one-of-list probs-height last ; a random one of the [ntags p-height] of the list
  ;
  ; OR the same probability for every type of pallet
  ;report first item (random 11) probs
end
...
to-report generate-number [ s ]
  report ifelse-value (s = "tags") [
    ; pallets with a poisson mean 60 and between 40 and 80
    ;random-tween-uniform 100 1000
    ;30
    ; min (list max (list 40 random-poisson mean-tags-per-pallet) 80)
    ;gen-num-tags-and-height
    ;(list 50 1.60)
    ;(list 63 1.862)
    ifelse-value multi-size-batches? [gen-num-tags-and-height][
      ;(list 63 1.862)
      (list 1 0.25)
    ]
  ] [
    ...
    ;1
    ;ifelse-value ( ((1 + random 10) * 10) >= short-EILU-percent) [ random-tween-uniform 27 33 ] [ 18 ]
    ;ifelse-value ( (1 + random 100) >= short-EILU-percent) [ 33 ] [ 18 ]
    ;first (rnd:weighted-one-of-list [[33 0.6] [18 0.4]] last)
    ifelse-value multi-size-batches? [27] [1]
  ]
end
...
to arrive
  ...
  let n-tags-h-list (list )
  create-pallets [ total-pallets ] of current-truck  [
    set n-tags-h-list (generate-number "tags")
    set num-tags-in-pallet first n-tags-h-list ;(generate-number "tags")
    set p-height-load last n-tags-h-list
    set pallets-of-this-truck (lput self pallets-of-this-truck)
    set pallet-distance base-point + (random-tween -0.4 0.4 )
    ...
    ;set tg-samples (tg-samples + 1)
    ;set avg-tg (avg-tg * (tg-samples - 1) / tg-samples + (num-tags-in-pallet / tg-samples))
  ]
  ask current-truck [
...
to setup-tags [ ?pallet ?truck]
  ...
  hatch-tags ([num-tags-in-pallet] of ?pallet) [
    ...
    set hidden? true
    set tag-x (random-tween (-1 * (p-width / 2)) (p-width / 2) )
    set tag-z (random-tween (-1 * (p-depth / 2)) (p-depth / 2) )
    set tag-y (random-tween p-base ([p-height-load] of ?pallet + p-base) ) ;p-height )
    ;set tag-x (random-tween -0.40 0.40 )
    ;set tag-y ( random-tween 0.3 0.75 )
    ;set tag-z ( random-tween -0.40 0.40 )
...
to-report compute-interf-power
  ...
  if (r2r-power-rx > interf-threshold) [; there would be interferences
    ...
    ;if (self = reader 0 or self = reader 23) and (myself = reader 0 or myself = reader 23) [show (list myself self r2r-power-rx) ]
  ]
...
to update-usage-stats [event-time] ;event-time is the next event time <<--OJO with NAMES
  ...
  let in-queue ifelse-value multiple-input-queues? [
    mean [length queue-r] of readers
  ][
    length queue
  ] ;;
  let in-queue-tags ifelse-value multi-size-batches? [sum [ sum [num-tags-in-pallet] of pallets-in-truck] of (turtle-set queue)] [ in-queue ]
  set total-tag-queue-time ( total-tag-queue-time + delta-time * in-queue-tags )
...
to make-final-results
  ...
  set avg-queue-length-tags ( total-tag-queue-time / (ticks - stats-start-time) )
  ...
  set avg-time-in-system-of-tags ifelse-value (total-system-throughput > 0) [( total-time-in-system / total-system-throughput )][0]
...
751
699
European Container Standard = European Intermodal Loading Unit (EILU).\n- Long EILU 13.2m long x 2.4m wide and carry up to 33 europallets.\n- Short EILU 7.2m long x 2.4m wide carry up to 18 europallets
8
0.0
1
...
165
818
not multi-size-batches? \n-> [63 tags, height 1.862m]
9
0.0
1
...
421
Queue Len tags
total-tag-queue-time / (ticks - stats-start-time)
2
1
9
...
@#$#@#$#@
<experiments>
  <experiment name="LBT1_30_0.1_to_1" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.1" step="0.1" last="1"/>
  ...
  <experiment name="LBT2_30_0.3_to_1" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-tags-inventoried</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.3" step="0.1" last="1"/>
    <enumeratedValueSet variable="size-of-frame">
      <value value="&quot;DFSA&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-time-of-tag-powered">
  ...
  <experiment name="LBT3_30_11_to_17" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="1.1" step="0.1" last="1.7"/>
    <enumeratedValueSet variable="size-of-frame">
      <value value="&quot;DFSA&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-time-of-tag-powered">
  ...
  <experiment name="sims-1Reader-Q4" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
    ...
    <metric>mean [cs-t] of readers / ticks</metric>
    <metric>mean [cc-t] of readers / ticks</metric>
    <metric>tag-unique-with-errors</metric>
    <metric>tag-with-interf-decod-well</metric>
    <metric>tag-with-interf-decod-bad</metric>
    <metric>tags-with-one-outage</metric>
    <metric>tags-with-two-outage</metric>
    <enumeratedValueSet variable="mean-tags-per-pallet">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Q">
    ...
    <enumeratedValueSet variable="distance-var">
      <value value="10"/>
    </enumeratedValueSet>
    <steppedValueSet variable="mean-arrival-rate" first="5" step="0.25" last="6.5"/>
  ...
  <experiment name="sims-1Reader-Q6" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
  ...
  <experiment name="sims-1Reader-Q7" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
  ...
  <experiment name="sims-1Reader-Q4-2" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="3" step="0.25" last="4.5"/>
    <enumeratedValueSet variable="two-readers?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="size-of-frame">
  ...
  <experiment name="sims-1Reader-Q6-2" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="3" step="0.25" last="4.75"/>
    <enumeratedValueSet variable="two-readers?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="size-of-frame">
  ...
  <experiment name="sims-1Reader-Q7-2" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
  ...
  <experiment name="sims-10Reader-Q4" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.05" step="0.05" last="0.5"/>
    <enumeratedValueSet variable="two-readers?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="size-of-frame">
  ...
  <experiment name="sims-10Reader-Q6" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
  ...
  <experiment name="sims-10Reader-Q7" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
  ...
  <experiment name="sims-10Reader-Q5" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
  ...
  <experiment name="sims-10Reader-Q3" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
  ...
  <experiment name="sims-10Reader-DFSA" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
  ...
  <experiment name="sims-40R-Q5-logi-test" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
  ...
  <experiment name="sims-40R-logi-test1" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-system-throughput-sg</metric>
    <metric>total-pallets-thoughput-sg</metric>
    ...
    <enumeratedValueSet variable="short-EILU-percent">
      <value value="40"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="two-readers?">
      <value value="false"/>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.05" step="0.025" last="0.4"/>
    <enumeratedValueSet variable="max-time-of-tag-powered">
      <value value="3.5"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="sims-40R-logi-Q5" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.01" step="0.01" last="0.5"/>
    <enumeratedValueSet variable="p-width">
      <value value="0.8"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="short-EILU-percent">
  ...
  <experiment name="sims-40R-logi-Q6" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-offered-load-of-tags</metric>
    <metric>total-offered-load-of-tags-sg</metric>
  ...
  <experiment name="sims-40R-logi-Q7" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-offered-load-of-tags</metric>
    <metric>total-offered-load-of-tags-sg</metric>
  ...
  <experiment name="sims-40R-logi-Q8" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-offered-load-of-tags</metric>
    <metric>total-offered-load-of-tags-sg</metric>
  ...
  <experiment name="sims-40R-logi-Q9" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-offered-load-of-tags</metric>
    <metric>total-offered-load-of-tags-sg</metric>
  ...
  <experiment name="sims-40R-logi-DFSA" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>total-offered-load-of-tags</metric>
    <metric>total-offered-load-of-tags-sg</metric>
  ...
  <experiment name="sims-40R-logi-Q10" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <enumeratedValueSet variable="mean-tags-per-pallet">
      <value value="60"/>
  ...
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
    <metric>total-offered-load-of-tags-sg</metric>
    <metric>total-offered-load-of-pallets</metric>
    <metric>total-offered-load-of-pallets-sg</metric>
    ...
    <metric>reader-utilization-percent</metric>
    <metric>avg-queue-length</metric>
    <metric>avg-queue-length-tags</metric>
    <metric>final-length-queue</metric>
    <metric>total-num-arrivals</metric>
    <metric>avg-time-queue-of-tags</metric>
    <metric>avg-time-in-system-of-tags</metric>
    ...
    <enumeratedValueSet variable="multi-size-batches?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="p-base">
      <value value="0.2"/>
  ...
  <experiment name="DFSA_on_bak" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.01" step="0.01" last="0.4"/>
    <enumeratedValueSet variable="distance-var-y">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-time-of-tag-powered">
  ...
  <experiment name="DFSA_on" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="DFSA_off" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="Q6_on" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="Q0_off" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="Q7_on" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="Q1_off" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="Q8_on" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="Q2_off" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="Q9_on" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="Q3_off" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
  ...
  <experiment name="Q6b_on" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.22" step="0.01" last="0.4"/>
    <enumeratedValueSet variable="distance-var-y">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-time-of-tag-powered">
  ...
  <experiment name="Q7b_on" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.25" step="0.01" last="0.3"/>
    <enumeratedValueSet variable="distance-var-y">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-time-of-tag-powered">
  ...
  <experiment name="Q7c_on" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.31" step="0.01" last="0.4"/>
    <enumeratedValueSet variable="distance-var-y">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-time-of-tag-powered">
  ...
  <experiment name="DFSA_on_test" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-arrival-rate</metric>
    <metric>total-offered-load-of-tags</metric>
    ...
    <steppedValueSet variable="mean-arrival-rate" first="0.28" step="0.01" last="0.35"/>
    <enumeratedValueSet variable="distance-var-y">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-time-of-tag-powered">

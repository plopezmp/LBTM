;; Version of 9 -Ago-2019 problem because cicles start at same time for all readers
;; starvation when network is large because some readers do not re-schedule LM time
;;
...
tags-own [
  ...
  time-of-inventory
  ;
  ;distance-to-reader_1 ; there are two readers per docking portal
  ;distance-to-reader_2
  distance-to-reader
...
;    set current-r-height ([ reader-height_1 ] of myself )
;    set D 3.5 ;D_1
;    set distance-to-r  distance-to-reader_1
;  ] [
;    set current-r-height ([ reader-height_2 ] of myself )
;    set D 3.5; D_2
;    set distance-to-r  distance-to-reader_2
;  ]
;
...
to-report generate-number [ s ]
  report ifelse-value (s = "tags") [
    ...
    ;ifelse-value two-height? [100][50]
    ;FOR this tests I am going to take [[70 1.995] ]
    ;ifelse-value two-height? [140][70]
    ;
    mean-tags-per-pallet
    ;ifelse-value two-height? [126][63] ; [[63 1.862] ]
    ;ifelse-value two-height? [200][100] ; [[63 1.862] ]
  ] [
...
;let lx-pallets (range  ( -1 * num-pallets-by-side ) (num-pallets-by-side + 1) )
;let ly-pallets lx-pallets
...
to-report compute-reader-gain-interf [ other-reader D distance-to-r ]
  ...
  ;set phi 90 - phi
  ;
  let alpha inclination_angle_1_tx_rx + 90 - asin ( (D / 2) / ((distance-to-r / 2) ) ) ;+ 90
  let phi alpha
...
to-report compute-interf-power
  ...
  let current-r-height ceiling-to-floor-distance   ;reader-height_1_tx_rx ; 1.5

  let D (distance myself) ; distance to myself
  set D (D * distance-multiplicative)
  let height-of-myself ceiling-to-floor-distance ;([reader-height_1_tx_rx] of myself)
  let distance-to-r (2 * sqrt ( ceiling-to-floor-distance ^ 2 + (D / 2) ^ 2 ) ) ;sqrt ( ( current-r-height - height-of-myself ) ^ 2 + (D ^ 2) )
  ...
  let reader-gain-tx-interf ( compute-reader-gain-interf myself D  distance-to-r )
  let this-reader self
  let reader-gain-rx-interf reader-gain-tx-interf
  ask myself [ set reader-gain-rx-interf ( compute-reader-gain-interf this-reader  D  distance-to-r ) ]
  ;
  ;;-----Rician only-------
  ;let tag-power-rx-linear ( ([ reader-power-tx * reader-gain-tx-rx * ((c / reader-frequency) ^ 2) ] of myself ) * tag-gain * polarz-mismatch * pow-tx-coefficient )  /
...
to make-final-results
  ...
  set final-length-queue (length queue)
...
scenario
scenario
"a" "b" "c" "d" "e" "f" "cycle_10" "cycle_20" "cycle_30" "cycle_40" "cycle_50" "cycle_60" "cycle_70" "cycle_80" "cycle_90" "cycle_100" "cycle_120" "cycle_140" "cycle_160" "cycle_180" "cycle_200"
10
